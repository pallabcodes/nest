...
import type { RouteDefinition, RouteMetadata } from '../types/route.js';
...
export class RouteDiscovery {
  ...
  private readonly discoveredRoutes = new Map<string, RouteDefinition>();

  /**
   * Discover routes from a list of controller classes.
   */
  discoverRoutes(controllers: readonly Constructor[]): RouteDefinition[] {
   ...
   */
  private discoverControllerRoutes(controller: Constructor): RouteDefinition[] {
    const routes: RouteDefinition[] = [];
    ...
    for (const [methodName, methodMetadata] of Object.entries(methods)) {
      if (methodMetadata.routes) {
        for (const routeMetadata of methodMetadata.routes) {
          const routeDefinition = this.createRouteDefinition(
            controller,
            methodName,
            routeMetadata,
            controllerPrefix
   ...
   */
  private resolveRoutePath(controllerPrefix: string, methodPath: string): string {
    // Normalize paths
   ...
   */
  private validateRouteConflicts(routes: RouteDefinition[]): void {
    ...
    for (const route of routes) {
      ...
      if (routeMap.has(key)) {
        ...
        throw new Error(
          `Route conflict detected: ${route.method} ${route.path} is already registered.\n` +
   ...
   */
  getDiscoveredRoutes(): RouteDefinition[] {
   ...
   */
  discoverControllerRoutesPublic(controller: Constructor): RouteDefinition[] {
   ...
   */
  getRoutesByController(): Map<Constructor, RouteDefinition[]> {
    const routesByController = new Map<Constructor, RouteDefinition[]>();
...
export async function registerControllerRoutes(
  ...
  adapter: any // HttpAdapter interface
): Promise<void> {
  ...
  for (const route of routes) {
    ...
    const result = await adapter.registerRoute(
      adapter.server || adapter.getHttpServer(),
      route,
      handler
    );
 ...
 */
function createControllerHandler(route: RouteDefinition): any {
  return {
    handle: async (request: any, reply: any) => {
      try {
 ...
 */
export function createRouteDiscovery(): RouteDiscovery {
 ...
 */
export function validateController(controller: Constructor): boolean {
  const discovery = new RouteDiscovery();
 ...
 */
export function getRouteInfo(routes: RouteDefinition[]): any[] {
