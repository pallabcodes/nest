...
// Note: Fastify types are imported dynamically to avoid bundling issues
...
import type { HttpAdapter, HttpAdapterConfig, RouteHandler } from '../types/adapter.js';
import type { Result } from '../types/results.js';
import type { Port } from '../types/branded.js';
 ...
 */
export class FastifyAdapter implements HttpAdapter<FastifyInstance, FastifyRequest, FastifyReply> {
   ...
   */
  async initialize(config: HttpAdapterConfig): Promise<Result<FastifyInstance>> {
    try {
   ...
   */
  async start(server: FastifyInstance, port: Port): Promise<Result<void>> {
    try {
      await server.listen({
        port: port as number,
   ...
   */
  async listen(port: Port, host?: string): Promise<Result<void>> {
    if (!this.server) {
   ...
   */
  async registerRoute(server: FastifyInstance, route: RouteDefinition, handler: RouteHandler<FastifyRequest, FastifyReply>): Promise<Result<void>> {
    try {
   ...
   */
  async stop(server: FastifyInstance): Promise<Result<void>> {
    try {
      await server.close();
      return { success: true, data: undefined };
    } catch (error) {
   ...
   */
  getHttpServer(): FastifyInstance | null {
    return this.server;
  }
   ...
   */
  async getHealth(server: FastifyInstance): Promise<Result<any>> {
    try {
      return {
        success: true,
        data: {
   ...
   */
  async registerMiddleware(_server: FastifyInstance, _middleware: any): Promise<Result<void>> {
   ...
   */
  private async importFastify(): Promise<any> {
    try {
      ...
      throw new Error(
        'Fastify not found. Please install it: npm install fastify\n' +
   ...
   */
  private registerGlobalHooks(server: FastifyInstance): void {
    ...
    server.addHook('onResponse', (request: any, reply: any, done: () => void) => {
      request.log.info({
        method: request.method,
        url: request.url,
   ...
   */
  private registerErrorHandler(server: FastifyInstance, _config: HttpAdapterConfig): void {
   ...
   * Convert Neat route definition to Fastify route options.
   */
  private convertRouteDefinition(route: RouteDefinition, handler: RouteHandler<FastifyRequest, FastifyReply>): any {
    return {
   ...
   */
  private convertMetadataToSchema(metadata: any): any {
