...
import { METADATA_KEYS, type MetadataKey } from './keys.js';
import {
  getScannerConfig,
  shouldScanTypeScript,
  shouldScanJavaScript,
...
export class MetadataScanner {
   ...
   */
  scanClass(target: Constructor): ClassMetadataResult {
    const injectable = this.getMetadata<boolean>(METADATA_KEYS.INJECTABLE, target);
    const controller = this.getMetadata<ControllerMetadata>(METADATA_KEYS.CONTROLLER, target);
    const module = this.getMetadata<ModuleMetadata>(METADATA_KEYS.MODULE, target);
    const strategies = this.getMetadata<string[]>(METADATA_KEYS.STRATEGY_KEY, target);
    const onInit = this.getMetadata<LifecycleHook>(METADATA_KEYS.ON_INIT, target);
    const onDestroy = this.getMetadata<LifecycleHook>(METADATA_KEYS.ON_DESTROY, target);
  ...
  scanMethod(
    ...
    const routes = this.getMetadata<RouteMetadata[]>(METADATA_KEYS.ROUTE, methodTarget, propertyKey);
    const middlewares = this.getMetadata<MiddlewareMetadata[]>(METADATA_KEYS.MIDDLEWARE, methodTarget, propertyKey);
    const guards = this.getMetadata<GuardMetadata[]>(METADATA_KEYS.GUARD, methodTarget, propertyKey);
    const interceptors = this.getMetadata<InterceptorMetadata[]>(METADATA_KEYS.INTERCEPTOR, methodTarget, propertyKey);
   ...
   */
  scanClassMethods(target: Constructor): Record<string, MethodMetadataResult> {
    const methods: Record<string, MethodMetadataResult> = {};
  ...
  scanProperty(
    ...
    const inject = this.getMetadata<InjectMetadata>(METADATA_KEYS.INJECT, methodTarget, propertyKey);
    const strategy = this.getMetadata<StrategyMetadata>(METADATA_KEYS.STRATEGY, methodTarget, propertyKey);
   ...
   */
  scanClassProperties(_target: Constructor): Record<string, PropertyMetadataResult> {
   ...
   */
  hasMetadata(key: MetadataKey, target: object, propertyKey?: string): boolean {
    return propertyKey !== undefined
   ...
   */
  getMetadataKeys(target: object, propertyKey?: string): MetadataKey[] {
    try {
      return propertyKey !== undefined
        ? (Reflect.getMetadataKeys(target, propertyKey) as MetadataKey[])
        : (Reflect.getMetadataKeys(target) as MetadataKey[]);
    } catch {
      return [];
    }
  }
   ...
   */
  clearMetadata(target: object, propertyKey?: string): void {
   ...
   */
  async scanForEntities(): Promise<Constructor[]> {
   ...
   */
  async scanForSchemas(): Promise<any[]> {
 ...
 */
export function createMetadataScanner(): MetadataScanner {
   ...
   */
  export function getControllerPrefix(target: Constructor): string | undefined {
    const scanner = createMetadataScanner();
   ...
   */
  export function getModuleProviders(target: Constructor): readonly Constructor[] {
    const scanner = createMetadataScanner();
   ...
   */
  export function getModuleControllers(target: Constructor): readonly Constructor[] {
    const scanner = createMetadataScanner();
   ...
   */
  export async function scanForEntities(): Promise<Constructor[]> {
   ...
   */
  export function scanForServices(): Constructor[] {
   ...
   */
  export function scanForControllers(): Constructor[] {
   ...
   */
  export async function scanForSchemas(): Promise<any[]> {
    ...
    try {
      ...
      // Remove duplicates
      const uniqueSchemas = schemas.filter((schema, index, arr) =>
        arr.findIndex(s => s.name === schema.name) === index
      );
   ...
   */
  function scanDirectoryForEntities(dir: string): Constructor[] {
    const entities: Constructor[] = [];
    const config = getScannerConfig();
   ...
   */
  function scanFileForEntities(filePath: string): Constructor[] {
    const entities: Constructor[] = [];
        ...
        try {
          ...
          for (const [key, value] of Object.entries(module)) {
            if (isEntityClass(value)) {
              entities.push(value as Constructor);
            }
          }
   ...
   */
  function scanTypeScriptFileForEntities(filePath: string): Constructor[] {
    const entities: Constructor[] = [];
   ...
   */
  function scanFileForEntityRegistrations(filePath: string): Constructor[] {
    const entities: Constructor[] = [];
   ...
   */
  function resolveEntityReference(ref: string, module: any): any {
   ...
   */
  function scanDirectoryForServices(dir: string): Constructor[] {
    const services: Constructor[] = [];
   ...
   */
  function scanFileForServices(filePath: string): Constructor[] {
    const services: Constructor[] = [];
        ...
        try {
          ...
          for (const [key, value] of Object.entries(module)) {
            if (isServiceClass(value)) {
              services.push(value as Constructor);
            }
          }
          return services;
        } catch (error) {
   ...
   */
  function scanTypeScriptFileForServices(filePath: string): Constructor[] {
    const services: Constructor[] = [];
   ...
   */
  function scanDirectoryForControllers(dir: string): Constructor[] {
    const controllers: Constructor[] = [];
   ...
   */
  function scanFileForControllers(filePath: string): Constructor[] {
    const controllers: Constructor[] = [];
        ...
        try {
          ...
          for (const [key, value] of Object.entries(module)) {
            if (isControllerClass(value)) {
              controllers.push(value as Constructor);
            }
          }
          return controllers;
        } catch (error) {
   ...
   */
  function scanTypeScriptFileForControllers(filePath: string): Constructor[] {
    const controllers: Constructor[] = [];
   ...
   */
  function scanDirectoryForSchemas(dir: string): any[] {
    const schemas: any[] = [];
   ...
   */
  function scanFileForSchemas(filePath: string): any[] {
    const schemas: any[] = [];
          ...
          for (const [key, value] of Object.entries(module)) {
            if (typeof value === 'object' && value !== null && isSchemaClass(value)) {
              ...
              if ('name' in valueAsObject) {
                schemas.push({
                  name: (valueAsObject as any).name,
   ...
   */
  function scanTypeScriptFileForSchemas(filePath: string): any[] {
    const schemas: any[] = [];
   ...
   */
  function scanFileForSchemaRegistrations(filePath: string): any[] {
    const schemas: any[] = [];
   ...
   */
  function resolveSchemaReference(ref: string, module: any): any {
    if (module[ref]) {
      return module[ref];
    }
    return null;
   ...
   */
  function isEntityClass(obj: any): boolean {
   ...
   */
  function isServiceClass(obj: any): boolean {
    if (typeof obj !== 'function' || !obj.prototype) {
      return false;
    }
   ...
   */
  function isControllerClass(obj: any): boolean {
    if (typeof obj !== 'function' || !obj.prototype) {
      return false;
    }
   ...
   */
  function isSchemaClass(obj: any): boolean {
    if (typeof obj !== 'function' || !obj.prototype) {
      return false;
    }
   ...
   */
  function getManuallyRegisteredEntities(): Constructor[] {
   ...
   */
  function getManuallyRegisteredSchemas(): any[] {
   ...
   */
  export function scanForGuards(): Array<[string, Constructor]> {
    const config = getScannerConfig();
    ...
    if (cached) {
      return cached as Array<[string, Constructor]>;
    }

    const results: Array<[string, Constructor]> = [];
   ...
   */
  export function scanForPipes(): Array<[string, Constructor]> {
    const config = getScannerConfig();
   ...
   */
  export function scanForInterceptors(): Array<[string, Constructor]> {
    const config = getScannerConfig();
   ...
   */
  export function scanForExceptionFilters(): Array<[string, Constructor]> {
    const config = getScannerConfig();
   ...
   */
  function isGuardClass(obj: any): boolean {
    if (typeof obj !== 'function' || !obj.prototype) {
      return false;
    }
   ...
   */
  function isPipeClass(obj: any): boolean {
    if (typeof obj !== 'function' || !obj.prototype) {
      return false;
    }
   ...
   */
  function isInterceptorClass(obj: any): boolean {
    if (typeof obj !== 'function' || !obj.prototype) {
      return false;
    }
   ...
   */
  function isExceptionFilterClass(obj: any): boolean {
    if (typeof obj !== 'function' || !obj.prototype) {
      return false;
    }
