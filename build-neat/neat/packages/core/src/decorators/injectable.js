...
export function Injectable(options = {}) {
    return (target) => {
        ...
        if (options.scope) {
            scanner.setMetadata(`${METADATA_KEYS.INJECTABLE}:scope`, options.scope, target);
        }
        // Attach global metadata if specified
        if (options.global) {
            scanner.setMetadata(`${METADATA_KEYS.INJECTABLE}:global`, options.global, target);
        }
        // Attach custom token if specified
        if (options.token) {
            scanner.setMetadata(`${METADATA_KEYS.INJECTABLE}:token`, options.token, target);
        }
...
export function getInjectableScope(target) {
    const scanner = new MetadataScanner();
    return scanner.getMetadata(`${METADATA_KEYS.INJECTABLE}:scope`, target);
}
/**
...
export function getInjectableToken(target) {
    const scanner = new MetadataScanner();
    return scanner.getMetadata(`${METADATA_KEYS.INJECTABLE}:token`, target);
}
/**
 ...
 * ```
 */
export function isInjectableGlobal(target) {
    const scanner = new MetadataScanner();
    return scanner.getMetadata(`${METADATA_KEYS.INJECTABLE}:global`, target) === true;
}
