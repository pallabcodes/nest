...
/**
 * Role type that can be a string or an object with a name property
 */
type RoleType = string | { name: string } | { name?: string };
...
export class RolesGuard implements CanActivate {
  ...
  private checkUserRoles(
    user: AuthenticatedUser | { roles?: RoleType[]; role?: string; roleNames?: string[] },
    ...
    if ('role' in user && user.role && typeof user.role === 'string') {
      return this.checkUserRolesStr(user.role, normalizedRequiredRoles);
    }

    if ('roleNames' in user && user.roleNames && Array.isArray(user.roleNames)) {
   ...
   */
  private checkUserRolesArr(userRoles: RoleType[], normalizedRequiredRoles: string[]): boolean {
  ...
  private getUserRoles(
    ...
    if ('roles' in user && user.roles && Array.isArray(user.roles)) {
      return user.roles.map((role: RoleType) =>
